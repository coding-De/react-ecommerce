...
import { fetchProductByIdAsync, selectedProduct } from './productListSlice'
...
export default function ProductDetail() {
  ...
  useEffect(()=>{
    if (product && items.findIndex((item) => item.id === product.id) >= 0) {
      setItemAdded(true);
    }else{
      setItemAdded(false);
    }
  ...
  const handleClick = (e) => {
    ...
    if (items.findIndex((item) => item.id === product.id) < 0) {
      ...
      // }
      dispatch(addToCartAsync({ ...product, item: newItem }));
      setItemAdded(true);
    } else {
      if (items.length)
